...
from onad.model.iforest.mondrian import MondrianForest, MondrianTree, MondrianNode
...
class TestMondrianForest(unittest.TestCase):
    ...
    def test_score_normalization(self):
        ...
        test_points = [
            {"feature_0": 0.0, "feature_1": 0.0, "feature_2": 0.0, "feature_3": 0.0},  # Normal
            {"feature_0": 100.0, "feature_1": 100.0, "feature_2": 100.0, "feature_3": 100.0},  # Anomaly
        ...
        for point in test_points:
            ...
            self.assertGreaterEqual(score, 0.0, f"Score {score} below 0 for point {point}")
    ...
    def test_reproducibility_with_seed(self):
        ...
        for s1, s2 in zip(scores1, scores2):
            self.assertAlmostEqual(s1, s2, places=10, 
                                 msg=f"Scores differ with same seed: {s1} != {s2}")
    ...
    def test_different_seeds_produce_different_results(self):
        ...
        different_count = sum(1 for s1, s2 in zip(scores1, scores2) if abs(s1 - s2) > 1e-10)
        self.assertGreater(different_count, len(scores1) * 0.5, 
