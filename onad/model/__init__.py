...
def __getattr__(name: str) -> Any:
    ...
    if name == "NullModel":
        module = importlib.import_module("onad.model.null")
        return module.NullModel
    if name == "RandomModel":
        module = importlib.import_module("onad.model.random")
        return module.RandomModel
    if name == "ThresholdModel":
        module = importlib.import_module("onad.model.threshold")
        return module.ThresholdModel
    if name == "QuantileThreshold":
        module = importlib.import_module("onad.model.quantile_threshold")
        return module.QuantileThreshold
    raise AttributeError(f"module 'onad.model' has no attribute '{name}'")
